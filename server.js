// server.js - COMPLETE FIX FOR HTTPS AND SECURITY ISSUES
...
let server;
if (isProduction && fs.existsSync('/etc/letsencrypt/live/yourdomain.com/privkey.pem')) {
    ...
    console.log('ℹ️ HTTP server created for development');
}
...
// Trust proxy in production for correct IP and protocol detection
if (isProduction) {
...
function startGame(game) {
    ...
    for (const socketId in game.players) {
        ...
        if (users[player.username] && !player.feeDeducted) {
            ...
            player.balance = users[username].balance;
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📍 Access your game at: ${isProduction ? 'https' : 'http'}://localhost:${PORT}`);
